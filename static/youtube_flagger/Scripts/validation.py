...
from ..Scripts import community_downloader as get_post_channel_url
import codecs
...
def validate_channel_id(inputted_channel):
        ...
        if startIndex < endIndex and endIndex <= len(inputted_channel):
            ...
            if response.get("items"):
                isolatedChannelID = response.get("items")[0]["snippet"][
                    "channelId"]  # Get channel ID from custom channel URL username
            else:
                print(
    ...
    # Handle legacy style custom URL (no /c/ for custom URL)
            ...
            inputted_channel.lower().startswith("youtube.com/") or str(urlparse(inputted_channel).hostname).lower() in [
    ...
    # Check if new "handle" identifier is used
        ...
        if re.match(r'^[a-zA-Z0-9._-]{3,30}$', inputted_channel[1:]):
            ...
            response = auth.YOUTUBE.search().list(part="snippet", q=inputted_channel, maxResults=1,
...
def validate_config_settings(config):
    ...
    def validate_chars(value, *args):
        if value == 'ask':
            return True
        result = utils.make_char_set(value, stripLettersNumbers=True, stripKeyboardSpecialChars=False,
    ...
    def validate_strings(value, *args):
        ...
        try:
            ...
            if len(result) > 0:
                ...
                print(
                    "The list appears empty! Make sure it is either a single string, or a comma separated list of them!")
                print_quit_and_report()
        except:
            print(f"\n{B.RED}{F.WHITE} ERROR! {S.R} Invalid value for config setting 'strings_to_filter': {str(value)}")
    ...
    validSettingsDict = {
        ...
        'none', 'id', 'username', 'text', 'nameandtext', 'autoascii', 'autosmart', 'sensitivesmart'),
    ...
    integerSettings = ['max_comments', 'recent_videos_amount', 'minimum_duplicates', 'quota_limit', 'config_version',
                       'stolen_minimum_text_length', 'minimum_duplicate_length']
