...
def get_authenticated_service(client_secrets, email):
    ...
    # global CLIENT_SECRETS_FILE
    # with open(os.path.join(settings.STATIC_URL, 'youtube_flagger/client_secrets.json')) as file:
    # CLIENT_SECRETS_FILE = '../client_secrets.json'
    # CLIENT_SECRETS_FILE = json.load(file)
    # print("client Secret file " + secrets_path)

    # CLIENT_SECRETS_FILE = '../../static/youtube_flagger/client_secrets.json'
    ...
    try:
        ...
        try:
            channelTitle = results["items"][0]["snippet"][
                "title"]  # If channel ID was found, but not channel title/name
        except KeyError:
            print(
            ...
            print(
                "> NOTE: The program may still work - You can try continuing. Just check the channel ID is correct: " + str(
                    channelID))
            channelTitle = ""
